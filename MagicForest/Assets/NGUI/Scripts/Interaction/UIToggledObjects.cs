...
{
  [Tooltip("勾选后当Toggle被取消选中时，会隐藏选中时active的Objects")]
  public bool HideWhenToggleFalse = false;
    ...
    else if(enabled && false == val && true == HideWhenToggleFalse)
    {
      for (int i = 0; i < activate.Count; ++i)
        Set(activate[i], val);
