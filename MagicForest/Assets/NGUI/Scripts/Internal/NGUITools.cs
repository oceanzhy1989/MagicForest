...
static public class NGUITools
{
  ...
  static public AudioSource PlaySound(AudioClip clip, float volume, float pitch)
  {
    ...
    if (clip != null && volume > 0.01f) {
      ...
      if (mListener != null && mListener.enabled && NGUITools.GetActive(mListener.gameObject)) {
        Camera audioSourceCamera = UICamera.mainCamera;
        if (null == audioSourceCamera) return null;
#if UNITY_4_3 || UNITY_4_5 || UNITY_4_6
				//AudioSource source = mListener.audio;
        AudioSource source = audioSourceCamera.GetComponent<AudioSource>();
#else
        AudioSource source = audioSourceCamera.GetComponent<AudioSource>();
#endif
        if (source == null) source = audioSourceCamera.gameObject.AddComponent<AudioSource>();
#if !UNITY_FLASH
        source.priority = 50;
        source.pitch = pitch;
#endif
  ...
  static public AudioSource StopSound()
  {
    float time = RealTime.time;
  ...
  /// <summary>
  /// DashFire.ResourceSystem.NewObject()生成的对象，用该方法添加到父节点上
  /// </summary>
