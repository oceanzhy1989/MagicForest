...
public class UIAtlasMaker : EditorWindow
{
  ...
  static public bool UpdateTexture(UIAtlas atlas, List<SpriteEntry> sprites)
  {
    ...
    if (PackTextures(tex, sprites))
    {
      ...
      if (atlas.useAlphaTexture)
      {
        alphaTex = new Texture2D(tex.width, tex.height, tex.format, false);
    ...
    // Generate Alpha Texture If Need
    if (atlas.useAlphaTexture && null != alphaTex)
    {
      Texture2D oldAlphaTex = atlas.alphaTexture as Texture2D;
      string oldAlphaPath = (oldAlphaTex != null) ? AssetDatabase.GetAssetPath(oldAlphaTex.GetInstanceID()) : "";
      string newAlphaPath = NGUIEditorTools.GetSaveableAlphaTexturePath(atlas);

      // Clear the read-only flag in texture file attributes
      if (System.IO.File.Exists(newAlphaPath))
      {
#if !UNITY_4_1 && !UNITY_4_0 && !UNITY_3_5
        if (!AssetDatabase.IsOpenForEdit(newAlphaPath))
        {
          Debug.LogError(newAlphaPath + " is not editable. Did you forget to do a check out?");
          return false;
        }
#endif
        System.IO.FileAttributes newAtlasPathAttrs = System.IO.File.GetAttributes(newAlphaPath);
        newAtlasPathAttrs &= ~System.IO.FileAttributes.ReadOnly;
        System.IO.File.SetAttributes(newAlphaPath, newAtlasPathAttrs);
      }

      bool newAlphaTex = (oldAlphaTex == null || oldAlphaPath != newAlphaPath);
      
      byte[] alphabytes = alphaTex.EncodeToPNG();
      System.IO.File.WriteAllBytes(newAlphaPath, alphabytes);
      alphabytes = null;

      // Load the texture we just saved as a Texture2D
      AssetDatabase.SaveAssets();
      AssetDatabase.Refresh(ImportAssetOptions.ForceSynchronousImport);
      alphaTex = NGUIEditorTools.ImportTexture(newAlphaPath, false, true, !atlas.premultipliedAlpha);

      if (newAlphaTex)
      {
        if (alphaTex == null) Debug.LogError("Failed to load the created atlas saved as " + newAlphaPath);
        else atlas.spriteMaterial.SetTexture("_AlphaTex", alphaTex);
	...
	void UpdateAtlas (List<Texture> textures, bool keepSprites, bool forceUpdate = false)
	{
		// Create a list of sprites using the collected textures
		List<SpriteEntry> sprites = CreateSprites(textures);

		if (sprites.Count > 0 || forceUpdate)
		{
    ...
    GUILayout.BeginHorizontal();
    NGUISettings.atlasUseAlphaTex = EditorGUILayout.Toggle("Use Alpha Tex", NGUISettings.atlasUseAlphaTex, GUILayout.Width(100f));
    GUILayout.EndHorizontal();
    ...
    bool fixedShader = false;
      ...
      string btnstr = "";
      if(NGUISettings.atlasUseAlphaTex != NGUISettings.atlas.useAlphaTexture)
      {
        btnstr += NGUISettings.atlasUseAlphaTex ? "[Create AlphaTex]" : "[Delete AlphaTex]";
      }
			if (textures.Count > 0)
			{
        btnstr += "[Add/Update Texture]";
      }
      if(btnstr.Length > 0)
      {
        update = GUILayout.Button(btnstr);
      }
		...
		else
		{
          ...
          {
            if(NGUISettings.atlasUseAlphaTex)
            {
              mat.EnableKeyword("USE_ALPHA_TEX");
            }
            else
            {
              mat.DisableKeyword("USE_ALPHA_TEX");
            }
          }
        ...
        {
          if (!NGUISettings.atlasPMA && null != NGUISettings.atlas.spriteMaterial && NGUISettings.atlasUseAlphaTex != NGUISettings.atlas.useAlphaTexture)
          {
            ...
            if (NGUISettings.atlasUseAlphaTex)
              UpdateAtlas(textures, true, true);
          }
          else
          {
            UpdateAtlas(textures, true, false);
          }
        }
  ...
  // set default values while atlas changed
  private void OnAtlasChange()
  {
    if (null == NGUISettings.atlas)
      return;
    NGUISettings.atlasUseAlphaTex = NGUISettings.atlas.useAlphaTexture;
  }
}
