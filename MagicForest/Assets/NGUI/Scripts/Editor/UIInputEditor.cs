...
public class UIInputEditor : UIWidgetContainerEditor
{
	public override void OnInspectorGUI ()
	{
		...
		EditorGUI.BeginDisabledGroup(label == null || label.objectReferenceValue == null);
		{
			...
			NGUIEditorTools.DrawEvents("On PreChange", input, input.onPreChange);
			NGUIEditorTools.DrawEvents("On Select", input, input.onSelect);
			NGUIEditorTools.DrawEvents("On Deselect", input, input.onDeselect);
			EditorGUI.EndDisabledGroup();
		}
		EditorGUI.EndDisabledGroup();
